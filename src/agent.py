...
class Agent:
    ...
    def handle_message(self, message_text, base64_images, base64_images_mimes):
        image_url = None if len(base64_images) == 0 else f"data:{base64_images_mimes[0]};base64,{base64_images[0]}"
        result = self._agent_executor_without_image.invoke({
            "input": message_text, "chat_history": self._chat_history
        }) if image_url == None else self._agent_executor_with_image.invoke({
